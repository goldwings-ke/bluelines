...
class Invoices extends Component {
  ...
  handleChange(e) {
    ...
    } else if(e.target.name === 'INVOICE_NO'){
          this.setState({[e.target.name]: e.target.value});   
  ...
  render() {
          ...
          var listItems =this.state.items.map((item) => 
            ...
            return (
              ...
              <span style={{float: 'right', color: "#2F4F4F", fontSize: "x-large"}}>{invoiceheader}</span><br/><br/>
                 <label>Address:<br/> 
                <textarea name="ADDRESS"  rows={4} wrap="soft" value={this.state.ADDRESS} onChange={this.handleChange} style={{width: "100%"}} />
                </label>
                <hr/>
                <span style={{color: "black"}}>To: {customer_name}</span><span style={{color: "blue", float: "right"}}>Invoice No:<input name="INVOICE_NO" value={this.state.INVOICE_NO} onChange={this.handleChange}/></span><br/>
                <span style={{color: "blue"}}>  Date:     {this.state.INVOICE_DATE_TXT}</span><span style={{float: "right"}}>Order No:{proformaNo}</span><br/>
                <label>MEMO:<br/> 
    ...
    return (
      <div className="w3-container" >
        <div className="w3-container" >
          <button onClick={() => this.setState({displayPanel: 'invoiceList'})} style={{marginLeft: "10px"}}>Invoices </button>
