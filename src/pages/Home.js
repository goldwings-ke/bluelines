...
class Home extends React.Component {
  ...
  initialize(){
   ...
   const dbRef = firebase.database().ref('currentUser/'+
    ...
    dbRef.on('value',  snapshot => {
      ...
      var newState = [];
        for (let item in items) {
            ...
            newState.push({
              id: item,
              currentUser: items[item].currentUser
            })
        }
          this.setState({
            currentUserArray: newState
          })
    });
    ...
    if(counter > 0){
      ...
      /*  
      else if(currentUserExists === false){
      // save first occuring business into currentUser path
        
        const dbRef2 = firebase.database().ref('currentUser/'+uid);
        const item = {
            currentUser: mycurrentUser
          }
         dbRef2.push(item);
  ...
  handleChange(e){
    ...
    } else if(e.target.name==='currentUser'){
        var mcurrentUser = e.target.id;

        var muid = this.state.uid;
  ...
  var newPostKey = firebase.database().ref().child('currentUser').push().key;
          var updates = {};
    updates['/currentUser/' + muid + '/' + newPostKey] = postData;
          firebase.database().ref().update(updates);
            this.setState({
              currentUserId: newPostKey,
              currentUser: mcurrentUser
            })
        } else {
            var key =this.state.currentUserId;
            const dbRef2 = firebase.database().ref('currentUser/'+uid+'/');
            dbRef2.child(key).update({
              [e.target.name]: mcurrentUser
              });
            this.setState({
              [e.target.name]: e.mcurrentUser
            })
        }
      } else {
        const myref = this.state.currentUserArray.map((cua) => {
          if(cua.currentUser === mcurrentUser){
            var id_delete = cua.id;
           const itemRef = firebase.database()
          .ref(`/currentUser/${muid}/${id_delete}`);
          itemRef.remove();
          }
        })
          this.setState({
      ...
      if(this.state.user == null)
       return <FirebaseAuth uiConfig={this.uiConfig} firebaseAuth={firebase.auth()}/>;
            ...
            if(this.state.displayPane === 'list'){
              ...
              <li key={item.id}>
               ...
               <p>Contact: <strong>{item.telephoneHome} {item.telephoneOffice}</strong></p>
                ...
                <input type="checkbox" name="currentUser" checked={item.id === this.state.currentUser} onChange={this.handleChange} id={item.id} />
               </p>
             </li>
              );
              return(
        ...
        <button class="w3-button w3-yellow" style={{float: "right"}} type = "submit" onClick={() => this.clear()}>Clear</button>
                ...
                <input class="w3-input" type="checkbox" name="allowClientQuotations" placeholder="Allow Client Quotations" checked={this.state.allowClientQuotations} value={this.state.allowClientQuotations} onChange={this.handleChange} />
 ...
 <div className="w3-container" >
  <div className="w3-container w3-teal" >
    ...
    <button onClick={() => this.handleClick(1)} style={{marginLeft: "10px"}}>Businesses</button>
