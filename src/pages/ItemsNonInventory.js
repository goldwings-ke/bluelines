...
class ItemsNonInventory extends React.Component {
  constructor(){
    super();
        this.state = {
  ...
  initialize2(){
   ...
   var businessKeyId =this.state.businessKeyId;
      ...
      if(e.target.value === ""){
       ...
       {this.state.items.filter(item => item.itemName.
        includes(searchTerm)).map(filteredItem => (
          ...
          id: filteredItem.id,
          imageName: filteredItems.imageName,
          imgURL: filteredItems.imgURL,          
   ...
   var businessKeyId =this.state.businessKeyId;
    ...
    var itemsRef = null;
      ...
      if(this.state.isExists){
        itemsRef = firebase.database().ref('noninventoryitems/'+uid+'/'+businessKeyId+'/'+this.state.id);
        saved = "Updated!";
        const item = {
        ...
        itemsRef = firebase.database().ref('noninventoryitems/'+uid+'/'+ businessKeyId);
        const item = {
          barCode: this.state.barCode, 
          businessKeyId:  this.state.businessKeyId,
          category: category,
...
export default ItemsNonInventory;