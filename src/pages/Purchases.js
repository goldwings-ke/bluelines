...
import BillsFixedAssets from './BillsFixedAssets';
import BillsInventory from './BillsInventory';
...
class Purchases extends React.Component {
  ...
  handleClick(index){
    ...
    if(index === 2)
        this.setState({
          displayPane: "expenses"
        })
    if(index === 3)
      this.setState({
        displayPane: "inventory"
      })
     if(index === 4)
      this.setState({
        displayPane: "fixedassets"
      })
  }
  ...
  render(){
    ...
    const renderDefault = () =>{
      ...
      else if(this.state.displayPane === "expenses")
      return(
        <Bills/>
      );
      else if(this.state.displayPane === "inventory")
      return(
        <BillsInventory/>
      );
      else if(this.state.displayPane === "fixedassets")
      return(
        <BillsFixedAssets/>
      );
      else{
       ...
       <div>  
        ...
        <div class="w3-row-padding">
          ...
          <div class="w3-col s6 w3-center" onClick={() => this.handleClick(3)}>
            <h3 >Inventory Purchases  &nbsp;
            <i class="fa fa-balance-scale"></i></h3>
          </div>
          <div class="w3-col s6 w3-center" onClick={() => this.handleClick(3)}>
            <h3 >Fixed Assets Purchases  &nbsp;
            <i class="fa fa-balance-scale"></i></h3>
          </div>
        </div>
       </div>
    ...
    <div className="w3-container" >
      <div className="w3-container w3-teal" >
        ...
        <button onClick={() => this.handleClick(1)} style={{marginLeft: "10px"}}>Purchases List</button>
      </div>
